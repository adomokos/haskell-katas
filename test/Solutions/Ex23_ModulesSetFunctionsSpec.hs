module Solutions.Ex23_ModulesSetFunctionsSpec
  ( spec
  ) where
...
spec = do
  describe "Set" $ do
    ...
    it "can find the difference" $ do
      ...
      Set.difference set2 set1 `shouldBe` Set.fromList "!Tbcgvw"
    it "can calculate the union" $ do
      Set.union set1 set2 `shouldBe`
        Set.fromList " !.?AIRTabcdefghijlmnorstuvwy"
    it "can use null, size, singleton, insert, delete" $ do
      ...
      (Set.insert 8 $ Set.fromList [1 .. 4]) `shouldBe`
        Set.fromList [1, 2, 3, 4, 8]
      (Set.delete 4 $ Set.fromList [3, 4, 5, 4, 3, 4, 5]) `shouldBe`
        Set.fromList [3, 5]
    it "can filter odd items from set" $ do
      (Set.filter odd $ Set.fromList [3, 4, 5, 6, 7, 2, 3, 4]) `shouldBe`
        Set.fromList [3, 5, 7]
