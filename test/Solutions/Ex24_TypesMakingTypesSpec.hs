module Solutions.Ex24_TypesMakingTypesSpec
  ( spec
  ) where
...
spec = do
  describe "Custom Types" $ do
    it "can calculate surface for two types" $ do
      ...
      (surface $ Rectangle (-10) 0 100 100) `shouldBe` 11000.0
      (surface $ Rectangle 0 0 (-100) (-100)) `shouldBe` 10000.0
    it "can print out the Shape by adding Show" $ do
      Circle 10 20 5 `shouldBe` Circle 10.0 20.0 5.0
    it "can use Value constructors as functions" $
      -- Use map function here
     do
      map (Circle 10 20) [4, 5] `shouldBe`
    ...
    it "can calculate surface for two types with Points" $ do
      ...
      (surface' $ Rectangle' (Point 0 0) (Point 100 100)) `shouldBe` 10000.0
    it "can nudge a Shape, still holding its dimension" $ do
      (nudge (Circle' (Point 0 0) 3) 2 4) `shouldBe` Circle' (Point 2.0 4.0) 3.0
      (nudge (Rectangle' (Point 0 0) (Point 4 4)) 2 4) `shouldBe`
        Rectangle' (Point 2.0 4.0) (Point 6.0 8.0)
    it "can nudge a Shape starting with base shapes" $ do
      (nudge (baseCircle 3) 2 4) `shouldBe` Circle' (Point 2.0 4.0) 3.0
      (nudge (baseRectangle 4 4)) 2 4 `shouldBe`
        Rectangle' (Point 2.0 4.0) (Point 6.0 8.0)
