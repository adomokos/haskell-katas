module Solutions.Ex33_TypesSequenceSpec
  ( spec
  ) where
...
spec =
  describe "Sequence" $ do
    ...
    it "can append and prepend" $ do
      1 <| Seq.singleton 2 `shouldBe` Seq.fromList [1, 2]
      Seq.singleton 1 |> 2 `shouldBe` Seq.fromList [1, 2]
    it "can combine two" $ do
      let left = Seq.fromList [1, 3, 3]
      let right = Seq.fromList [7, 1]
      left >< right `shouldBe` Seq.fromList [1, 3, 3, 7, 1]
