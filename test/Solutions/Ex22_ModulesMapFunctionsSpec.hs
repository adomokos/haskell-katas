module Solutions.Ex22_ModulesMapFunctionsSpec
  ( spec
  ) where
...
spec = do
  describe "Map module" $ do
    it "can convert a list to map with Map.fromList" $ do
      (length $ Map.fromList phoneBook) `shouldBe` 6
    ...
    it "returns 0 for size of empty map" $ do Map.size Map.empty `shouldBe` 0
    ...
    it "can map and filter values in a map" $ do
      (Map.map (* 100) $ Map.fromList [(1, 1), (2, 4), (3, 9)]) `shouldBe`
        (Map.fromList [(1, 100), (2, 400), (3, 900)])
      (Map.filter (`elem` ['A' .. 'Z']) $
       Map.fromList [(1, 'a'), (2, 'A'), (3, 'b'), (4, 'B')]) `shouldBe`
        (Map.fromList [(2, 'A'), (4, 'B')])
    it "can convert a Map to List" $
      -- Map.singleton initialize a map with a single value
      -- Map.insert adds 1 item to it
      -- Map.toList converts the map to List
     do
      (Map.toList . Map.insert 9 2 $ Map.singleton 4 3) `shouldBe`
        [(4, 3), (9, 2)]
